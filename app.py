...
def upload_and_process_cv():
    ...
    headers = final_order
    values = [cv_data.get(key) for key in final_order]
    return Response(json.dumps({"headers": headers, "row": values}, indent=4), mimetype='application/json')
...
def export_to_sheets():
    ...
    if not headers or not values:
        return Response(json.dumps({"error": "Incomplete data received."}), status=400, mimetype='application/json')

    try:
        spreadsheet = sheets_service.spreadsheets().create(
            body={"properties": {"title": f"CV Analysis - {values[0]}"}},
        ...
        return Response(json.dumps({"error": f"Failed to create sheet: {e}"}), status=500, mimetype='application/json')
